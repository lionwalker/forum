            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    ...
                    <div class="flex items-center justify-end mb-4">
                        <a href="{{ url("posts/add") }}" class="inline-flex items-center px-4 py-2 bg-gray-800 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-gray-700 active:bg-gray-900 focus:outline-none focus:border-gray-900 focus:ring ring-gray-300 disabled:opacity-25 transition ease-in-out duration-150">ADD POST</a>
                    </div>
                    <table class="table-auto min-w-full leading-normal">
                        ...
                        @foreach ($posts as $post)
                            <tr class="table-row">
                                <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">{{ Str::substr($post->title,0,50) }}</td>
                                <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">{{ Str::substr($post->post,0,100) }}</td>
                                <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm w-1/12">
                                    <a href="{{ $isAdmin ? url("/posts/approve/$post->id") : '#' }}" class="inline-block rounded-full text-white {{ $post->approved ? 'bg-green-400 hover:bg-green-500' : 'bg-red-400 hover:bg-red-500' }} duration-300 text-xs font-bold mr-1 md:mr-2 mb-2 px-2 md:px-4 py-1 opacity-90 hover:opacity-100">
